...
const providers = [
  {
    ...
    steps: [
      ...
      { title: "Sign in for cloud models", content: <><p>Cloud models (e.g. <code className="font-mono text-sm">nemotron-3-super:cloud</code>) run on Ollama&apos;s infrastructure for free but require a signed-in Ollama account:</p><CodeBlock language="bash" code={`# Sign in to your Ollama account (one-time setup)\nollama signin`} /><p className="text-sm text-muted-foreground mt-2">That&apos;s it — ZenCoder automatically pulls and launches cloud models for you. No need to run <code className="font-mono text-sm">ollama pull</code> manually. Sign-in is only required for <code className="font-mono text-sm">:cloud</code> variants; local models work without it.</p></> },
